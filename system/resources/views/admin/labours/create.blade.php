    ...
    @endif
      <div class="card-body">
        <div class="row">
            ...
            <div class="form-group">
               <div class="row">
                ...
                class="form-control @error('purpose')'is-invalid'@enderror">
                  ...
                  <option {{(old("purpose",(isset($row))?$row->purpose:"")=="labour")?"selected":""}} value="labour">Active Mine Labour  (<span class="urdu"> فعال کان کن </span>) </option>
                  <option {{(old("purpose",(isset($row))?$row->purpose:"")=="deceased labour")?"selected":""}} value="deceased labour">Labour Died in Mine Accident  (<span class="urdu">   مائن حادثے میں فوت شدہ کان کن  </span>)</option>
                  <option {{(old("purpose",(isset($row))?$row->purpose:"")=="permanent disabled")?"selected":""}} value="permanent disabled">Permanent Disabled Labour due to Mine Accident(<span class="urdu">    مائن حادثے کی وجہ سے مستقل معذور کان کن  </span>)</option>
                  <option {{(old("purpose",(isset($row))?$row->purpose:"")=="occupational desease")?"selected":""}} value="occupational desease">Mine Labour with Occupational Pulmonary Disease (<span class="urdu">   
          ...
          <div class="col-sm-6 {{(!old('purpose',(isset($row))?$row->purpose:"")||old('purpose',(isset($row))?$row->purpose:"")=='labour')?"hide":""}}" id="doa_date">
            @include("admin.layouts.partials.date",["name"=>"doa","title"=>"Date of Accident","date"=>$row->doa])
          </div>
    
          <div class="col-sm-6 {{(!old('purpose')||old('purpose')!='deceased labour')?"hide":""}}" id="death_date">
            @include("admin.layouts.partials.date",["name"=>"death_date","title"=>"Date of Death","date"=>$row->death_date])
          </div>
        ...
        <div class="row">
            ...
            <div class="form-group">
               <div class="row">
                ...
                <input 
                      type="text" 
                      name="cnic"
                      class="cnic digit-only form-control" 
                      id="cnic" 
                      readonly
                      value="{{old('cnic',(isset($row))?$row->cnic:"")}}"
          ...
          <div class="col-sm-6">
            @include("admin.layouts.partials.date",["name"=>"dob","title"=>"Date of Birth","date"=>$row->dob])
          </div>
          ...
          <div class="col-sm-6">
            <div class="form-group">
               <div class="row">
                ...
                <option {{(old("gender",(isset($row))?$row->gender:"")=='male')?"selected":""}} value="male">Male</option>
                <option {{(old("gender",(isset($row))?$row->gender:"")=='female')?"selected":""}} value="female">Female</option>
                </select>
          ...
          <div class="col-sm-6">
            <div class="form-group">
               <div class="row">
                ...
                <option {{(old('domicile_district',(isset($row))?$row->domicile_district:"")==$district->d_id)?"selected":""}} value="{{$district->d_id}}">{{$district->name}}</option>
                @endforeach
                </select>
          ...
          <div class="col-sm-6">
            <div class="form-group">
              <div class="row">
                ...
                <input class="form-check-input position-static" {{(old("married",(isset($row))?$row->married:"")=="yes")?"checked":""}} name="married" type="radio" value="yes"> Yes/ <span class="urdu">جی ہاں</span>
              </div>
              <div class="form-check">
                <!--data-dest="#family-info-container" data-show-value="yes"-->
                <input class="form-check-input position-static" {{(old("married",(isset($row))?$row->married:"")=="no")?"checked":""}}  name="married" type="radio" value="no"> No/ <span class="urdu">نہیں</span>
              </div>
              @error('married')
                <small class="text-danger">
                  {!! $message !!}
          ...
          <div class="col-sm-6">
            <div class="form-group">
              <div class="row">
                ...
                <div class="form-check"> 
                <input data-dest="#eobi_no" data-show-value="yes" class="form-check-input position-static" name="eobi" {{(old("eobi",(isset($row))?$row->eobi:"")=="yes")?"checked":""}} type="radio" value="yes"> Yes/ <span class="urdu">جی ہاں</span>
              </div>
              <div class="form-check">
                <input data-dest="#eobi_no" data-show-value="yes" class="form-check-input position-static" name="eobi" {{(old("eobi",(isset($row))?$row->eobi:"")=="no")?"checked":""}} type="radio" value="no"> No/ <span class="urdu">نہیں</span>
              </div>
              @error('eobi')
                <small class="text-danger">
                  {!! $message !!}
        ...
        <div class="row">
          ...
          <div class="col-sm-6">
            @include("admin.layouts.partials.date",["name"=>"work_from","title"=>"Work From","date"=>$row->work_from])
          </div>
          ...
          <div class="col-sm-6">
            <div class="form-group">
               <div class="row">
                ...
                name="work_type" 
                class="form-control" 
                data-dest="#other_work_type">
                  <option disabled="" selected="">Select Work Type</option>
                  @foreach ($worktypes as $key => $value)
                  <option {{($value['wt_id']==old('work_type',(isset($row))?$row->work_id:""))?"selected":""}} value="{{$value['wt_id']}}">{{$value['title']}}</option>	
                  @endforeach
                  <option {{(old('work_type')=="other")?"selected":""}} value="other">
                    <span class="english">Other</span> 
                    <span class="urdu" style="float: right;"> اگر آپ کا کام فہرست میں موجود نہیں ہے </span></option>
                </select>
              @error('work_type')
              <small class="text-danger">
                {!! $message !!}
          ...
          <div class="col-sm-6 {{(old('work_type')!='other')?"hide":""}}" id="other_work_type">
            <div class="form-group">
              <div class="row">
                ...
                class="form-control" 
                value="{{old('other_work_type')}}"
                placeholder="Other Nature of Work"/>
          ...
          <div class="col-sm-12">
            <div class="form-group">
              <div class="row">
                ...
                name="perm_address"
                class="form-control"
                placeholder="Permanent Address">{{old('perm_address',(isset($row))?$row->perm_address:"")}}</textarea>
            ...
            <div class="form-group">
               <div class="row">
                ...
                class="form-control">
                  ...
                  <option {{($value['d_id']==old('perm_district',(isset($row))?$row->perm_district_id:""))?"selected":""}} value="{{$value['d_id']}}">{{$value['name']}}</option>	
                  @endforeach
                </select>
            ...
            <div class="form-group">
               <div class="row">
                ...
                class="form-control">
                  <option disabled="" selected="">Select District</option>
                  @foreach ($districts as $key => $value)
                  <option {{($value['d_id']==old('postal_district',(isset($row))?$row->postal_district_id:""))?"selected":""}} value="{{$value['d_id']}}">{{$value['name']}}</option>	
                  @endforeach
                </select>
          ...
          <div class="col-sm-6">
            <div class="form-group">
              <div class="row">
                ...
                class="form-control" 
                id="lease_no" 
                value="{{old('lease_no',(isset($row))?$row->lease_no:"")}}"
            ...
            <div class="form-group">
              <div class="row">
                ...
                >
                  <option disabled="" selected="">Select District</option>
                  @foreach ($districts as $key => $value)
                  <option {{($value['d_id']==old('lease_district',(isset($row))?$row->lease_district_id:""))?"selected":""}} value="{{$value['d_id']}}">{{$value['name']}}</option>	
                  @endforeach
                </select>
          ...
          <div class="col-sm-6">
            <div class="form-group">
              <div class="row">
                ...
                <select 
                data-dest="#other_mineral_container" 
                name="mineral" 
                class="form-control"
                >
                  <option disabled="" selected="">Select Mineral</option>
                  @foreach ($minerals as $key => $value) { }}
                  <option {{($value['m_id']==old('mineral',(isset($row))?$row->mineral_id:""))?"selected":""}} value="{{$value['m_id']}}">{{$value['name']}}</option>	
                  @endforeach
          ...
          <div class="col-sm-6 {{(old('mineral')!='other')?"hide":""}}" id="other_mineral_container">
            <div class="form-group">
              <div class="row">
                ...
                name="other_mineral" 
                class="form-control" 
                value="{{old('other_mineral')}}"
                placeholder="Other Mineral"/>
          ...
          <div class="col-sm-12">
            <div class="form-group">
              <div class="row">
                ...
                name="lease_address" 
                class="form-control" 
                value="{{old('lease_address')}}"
                placeholder="Lease Address">{{old('lease_address',(isset($row))?$row->lease_address:"")}}</textarea>
            ...
            <div class="form-check">
              <div class="row">
                ...
                <div class="col-lg-6 text-right">
                    <label class="urdu">مذکورہ بالا معلومات میرے علم کے مطابق صحیح اور درست ہیں</label>
                </div>
                <div class="col-lg-12">
                  <hr>
                </div>
